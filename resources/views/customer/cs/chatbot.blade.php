...
<head>
    ...
    <!-- Custom CSS -->
    <link href="{{ asset('css/app.css') }}" rel="stylesheet"> {{-- Jika Anda punya CSS umum --}}
    <style>
...
<body>
    ...
    <script>
        ...
        const GET_ARTICLES_URL = "{{ route('cs.get_articles') }}";
        const START_LIVE_CHAT_URL = "{{ route('cs.start_live_chat') }}";
        const SUBMIT_TICKET_URL = "{{ route('cs.submit_ticket') }}";
        const CURRENT_USER_ID = {{ Auth::check() ? Auth::id() : 'null' }};
        const IS_USER_LOGGED_IN = {{ Auth::check() ? 'true' : 'false' }};
    </script>
    <script src="{{ asset('js/customer/cs_chatbot.js') }}"></script>
</body>
</html>
